...
function AnalysisPage() {
  ...
  const analyzeAudio = async (audioFile) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://web-production-7d56.up.railway.app'}/api/v1/audio/analyze`, {
        method: 'POST',
        body: formData,
      });
  ...
  const analyzeSpotifyTrack = async (trackId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://web-production-7d56.up.railway.app'}/api/v1/audio/analyze/spotify`, {
        method: 'POST',
        body: formData,
      });
